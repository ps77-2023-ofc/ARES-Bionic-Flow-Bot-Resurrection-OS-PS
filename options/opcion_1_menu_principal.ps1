...
$showOption1optionsMenu = {
    ...
    Write-Host "4. Descargar / instalar VLC Media Player"
    Write-Host "5. Descargar / instalar K-Lite Codec Pack"
    Write-Host "6. Descargar / instalar Rainmeter"
    Write-Host "7. Descargar / instalar Lively Wallpaper"
    Write-Host "8. Descargar / instalar 7-Zip"
    Write-Host "9. Descargar / instalar IDM (Internet Download Manager)"
    Write-Host "10. Descargar / instalar Microsoft PowerToys"
    Write-Host "11. Regresar al menú principal"
    Write-Host "-------------------------------------------------------------------------------------------------------------------"
}
        ...
        if ($approval -eq "S"){
            ...
            if ($compatibilityStatus -eq "system-is-compatible" -and $scriptExecutionStatus -eq "check-finished"){
                ...
                if ($compatPassedApproval -eq "S"){
                    Write-Host "¡Espléndido! Comenzando tarea 1 de 2: Descarga de Rainmeter..."
            ...
            elseif ($compatibilityStatus -eq "isFull_FullLegends_but_not_compatible"-and $scriptExecutionStatus -eq "check-finished"){
                ...
                if ($FullFLegendsbutNotCompatDisclaimerAccepted -eq "S"){
                    Write-Host "De acuerdo. Comenzando tarea 1 de 2: Descarga de Rainmeter..."
                    & "$installersFolderPath\instalador_rainmeter.ps1"
                }
