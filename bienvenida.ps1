...
$welcomeMessage = {
    ...
    Write-Host "¡Hola! Te doy la bienvenida."
    Write-Host "Soy ARES Bionic-Flow, el script tipo bot automatizado oficial de Resurrection OS,"
    ...
    Write-Host "- Puedo ayudarte a habilitar algunos parámetros útiles en el sistema, y así brindarte acceso a algunas" 
    Write-Host " herramientas o cosas útiles."
    Write-Host "Cabe aclararte que, en mi menú principal, luego de esta pantalla, habrá una opción que te permitirá habilitar la" 
    Write-Host "Protección del sistema (en caso de que no la tengas activada) y crear un punto de restauración, en caso de que" 
    Write-Host "alguna de las cosas que realizo cause algún daño en el sistema."
    Write-Host "¡Estoy aquí para ayudarte! Si me necesitas, abre el Menú Extendido de Resurrection OS y allí me encontrarás." 
    Write-Host "Dale a la opción 'Iniciar ARES Bionic-Flow Bot'."
    Write-Host ""
    Write-Host "Cuando estés listo para comenzar, responde abajo con una S y pulsa Enter para comenzar."
    Write-Host ""
}
$ready = Read-Host "¿Estás listo para comenzar?"
if ($isAdmin) {
    ...
    elseif ($ready -eq "N"){
        ...
        Write-Host "¡Ups, respuesta incorrecta! Inténtalo nuevamente."
        Start-Sleep -Seconds 4
        Clear-Host
    ...
    Write-Host "¡No puedo funcionar correctamente si no me ejecutas con privilegios de Administrador! Ya que muchas de las cosas que hago requieren realizar cambios en el sistema, que requieren de aprobación."
